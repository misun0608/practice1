...
public class MemberController {
	...
	public String userCheck(MemberVO memberVO, HttpSession session, 
		...
		if (res == 1)
		{
			...
			{
				writer.write("<script>alert('로그인 성공!!');location.href='./sungjuklist.su';</script>");
				//return "redirect:/main.me";
			}
			else
			{
				writer.write("<script>alert('로그인 성공!!');location.href='./sungjukinfo.su';</script>");
			}
		}
		else 
		{
	...
	public String updateMemberForm(MemberVO memberVO, Model model) throws Exception { 
		MemberVO vo = memberService.updateMemberForm(memberVO);
	...
	public String updateMember(MemberVO memberVO, HttpServletResponse response) throws Exception { 
		...
		if (res != 0)
		{
			writer.write("<script>alert('회원 수정 성공!!');");
			writer.write("location.href='./memberlist.me';</script>");
		}
		else
		{
			writer.write("<script>alert('회원 수정 실패!!');");
			writer.write("location.href='./memberupdateform.me';</script>");
		}
		return null;
	}
