...
public class SungjuckDAO {
	...
	public ArrayList<SungjuckVO> getSungjucklist(){
		...
		try {
			conn = JDBCUtil.getConnection();	// JDBCUtil import만 시켜주면 그냥 사용할 수 있음
	...
	// 성적 삭제(delete)
	public int deleteSungjuck(SungjuckVO sungjuckVO) {
		int result = 0;
		
		try {
			conn = JDBCUtil.getConnection();
			
			pstmt = conn.prepareStatement("delete from ssungjuck where id =?");
			pstmt.setString(1, sungjuckVO.getId());
			result = pstmt.executeUpdate();
